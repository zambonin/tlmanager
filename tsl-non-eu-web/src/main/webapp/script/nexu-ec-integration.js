var NexU = function() {
    ...
    function _initRunningPortAndVersion(scheme, successCallback, oldVersionDetectedCallback, notInstalledCallback, cannotLoadScriptCallback) {
        ...
        _initRunningPortAndVersionRecursive(scheme, successCallback, oldVersionDetectedCallback, notInstalledCallback, cannotLoadScriptCallback, 0, portsStr.split(","));
    }

    function _initRunningPortAndVersionRecursive(scheme, successCallback, oldVersionDetectedCallback, notInstalledCallback, cannotLoadScriptCallback, index, ports) {
        port = ports[index].trim();
        $.get(scheme + "://localhost:" + port + "/nexu-info", function(data) {
            runningPort = port;
            runningVersion = data.version;
            if (runningVersion.startsWith(latestVersion)) {
                $.getScript(scheme + "://localhost:" + port + "/nexu.js").done(function() {
                    isInitialized = true;
            ...
            if (index + 1 < ports.length) {
                _initRunningPortAndVersionRecursive(scheme, successCallback, oldVersionDetectedCallback, notInstalledCallback, cannotLoadScriptCallback, index + 1, ports);
            } else {
                isInitialized = true;
